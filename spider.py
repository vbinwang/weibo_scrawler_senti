...
def weibo_spider(keyword, maxpage=50, login=True, driver=None, username=None, password=None, browser='Chrome',
...
def get_url_by_time(keyword, time_from, time_to):
    ...
    url_mid = parse.quote(keyword)  # 将text转为URL编码
    return 'http://s.weibo.com/weibo/{mid}&scope=ori&timescope=custom:{timefrom}:{timeto}'.format(mid=url_mid, timefrom=time_from, timeto=time_to)
...
if __name__ == '__main__':
    csv_file = 'jiaxiang.csv'
    my_username = 'liweiba7430339662@163.com'
    my_password =  'QHEvzd911LP'
    keyword = '嘉祥外国语学校'
    my_browser = 'Chrome'
    ...
    # 按指定时间段爬取示例
    time_start = '2018-05-11-08'
    time_end = '2018-05-11-12'
    ...
    result = np.array(list(weibo_result.values())).T
    save_blog(result,csv_file)
    time_start = '2018-05-11-12'
    time_end = '2018-05-11-16'
    print('第二个开始啦')
    driver, weibo_result = weibo_spider(keyword=keyword, username=my_username, password=my_password, time_from=time_start, time_to=time_end,login=False,driver=driver)
    result = np.array(list(weibo_result.values())).T
    save_blog(result,csv_file)
    time_start = '2018-05-11-16'
    time_end = '2018-05-11-20'
    driver, weibo_result = weibo_spider(keyword=keyword, username=my_username, password=my_password, time_from=time_start, time_to=time_end,login=False,driver=driver)
    result = np.array(list(weibo_result.values())).T
    save_blog(result,csv_file)
    time_start = '2018-05-11-20'
    time_end = '2018-05-11-24'
    driver, weibo_result = weibo_spider(keyword=keyword, username=my_username, password=my_password, time_from=time_start, time_to=time_end,login=False,driver=driver)
    result = np.array(list(weibo_result.values())).T
    save_blog(result,csv_file)
    for date in range(12,25):
        for hour in range(0,24,4):
            time_start = '2018-05-%d-%d'%(date,hour)
            time_end = '2018-05-%d-%d'%(date,hour+4)
            driver, weibo_result = weibo_spider(keyword=keyword, username=my_username, password=my_password, time_from=time_start, time_to=time_end,login=False,driver=driver)
            result = np.array(list(weibo_result.values())).T
            save_blog(result,csv_file)
